...
{
    // Note, because this is called from a different thread than the mainthread, the thread un-safe ShowMessage is
    // better not used.
    /* MessageBoxA(
         0,
         "A debug event has happened. You could do some editing of the context here...",
         "Debug Event Plugin Example",
         MB_OK);*/
...
BOOL __stdcall addresslistplugin(PPLUGINTYPE0_RECORD SelectedRecord)
{
    ...
    //     x,
    //     100,
    //     "Selected record's description=%s Address=%0.8llx",
    //     SelectedRecord->description,
    //     (UINT64)SelectedRecord->address);
...
BOOL __stdcall memorybrowserplugin(
...
BOOL __stdcall CEPlugin_GetVersion(PPluginVersion pv, int sizeofpluginversion)
{
    ...
    pv->pluginname = "access v1.0"; // exact strings like this are pointers to the string in
                                    // the dll, so workable
    return TRUE;
}

int
lua_pluginExample(lua_State *L) // make sure this is cdecl
{
...
BOOL __stdcall CEPlugin_InitializePlugin(PExportedFunctions ef, int pluginid)
{
    ...
    init0.name = "access plugin: Addresslist";
    init0.callbackroutine = addresslistplugin;
    addresslistPluginID =
        Exported.RegisterFunction(pluginid, ptAddressList, &init0); // adds a plugin menu item to the memory view
    ...
    init1.name = "access plugin: Memoryview";
    init1.callbackroutine = memorybrowserplugin;
    init1.shortcut = "Ctrl+Q";
    memorybrowserpluginid =
        Exported.RegisterFunction(pluginid, ptMemoryView, &init1); // adds a plugin menu item to the memory view
    ...
    debugpluginID =
        Exported.RegisterFunction(pluginid, ptOnDebugEvent, &init2); // adds a plugin menu item to the memory view
    if (debugpluginID == -1)
    {
    ...
    ProcesswatchpluginID = Exported.RegisterFunction(
        pluginid, ptProcesswatcherEvent, &init3); // adds a plugin menu item to the memory view
    ...
    PointerReassignmentPluginID = Exported.RegisterFunction(
        pluginid, ptFunctionPointerchange, &init4); // adds a plugin menu item to the memory view
    ...
    init5.name = "access plugin: Main Menu";
    init5.callbackroutine = mainmenuplugin;
    init5.shortcut = "Ctrl+R";
    MainMenuPluginID =
        Exported.RegisterFunction(pluginid, ptMainMenu, &init5); // adds a plugin menu item to the memory view
    ...
    lua_register(lua_state, "pluginaccess", lua_pluginExample);
