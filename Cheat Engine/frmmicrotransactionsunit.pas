...
begin
  ...
  if internet=nil then
    internet:=TWinInternet.Create({$ifdef altname}'XXXX engine'{$else}strCheatEngine{$endif}+' microtransaction system');

  ss:=tstringstream.create({$if FPC_FULLVERSION<030200}''{$endif});
  try
    try
      internet.getURL('https://XXXXengine.org/microtransaction.php?action=buy&amount='+inttostr(tbitbtn(sender).Tag), ss);
